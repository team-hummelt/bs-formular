...
if ( ! class_exists( 'BootstrapFormularFilter' ) ) {
	...
	class BootstrapFormularFilter {
		...
		public function __construct() {
			...
			add_filter( 'bs_formular_validate_message_inputs', array(
				$this,
				'bsFormularValidateMessageInputs'
			), 10, 3 );
		...
		public function bs_form_create_formular_fields( $class_aktiv, $type, $label, $values, $case, $input_class, $label_class, $faIcon = false, $button_class = false ): object {
			...
			switch ( $case ) {
				...
				case'button':
					...
					$html                = $inputStart;
					$html                .= ' <button onclick="this.blur()" id="' . $id . '" name="' . $id . '" type="' . $values . '" class="btn ' . $btn . '">' . $faIcon . $label . '</button>';
					$html                .= $inputEnd;
					$record->html        = esc_textarea( $html );
					$record->inputId     = $id;
					$record->values      = $values;
				...
				case'dataprotection':
					...
					$linkLabel  = explode( '#', trim( $label ) );
					$html       = $inputStart;
					$html       .= '<div class="form-check dscheck">';
					$html       .= '<input onclick="this.blur()" class="form-check-input" data-id="' . $id . '" name="dscheck" type="checkbox" id="' . $id . '" ' . $checked . ' required>';
					$html       .= '<label class="form-check-label" for="' . $id . '">';
					//$html .= $label;
					$html            .= $linkLabel[0] . '<a href="' . $values . '">' . $linkLabel[1] . '</a>';
			...
			switch ( $type ) {
				case 'checkbox':
					...
					if ( $inputArr->required == 'required' ) {
						$return->status = false;
						//$this->lang = sanitize_text_field($inputArr->label);
						$msg         = apply_filters( 'bs_form_default_settings', 'by_field', $type );
						$return->msg = $msg->$type;
